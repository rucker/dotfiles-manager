...
def createInputFiles():
    with open(env.inputDir + Srcfiles.BASH_COMMON.value, 'w') as bashCommon:
        bashCommon.write('some_common_token=some_common_value\n')
    with open(env.inputDir + Srcfiles.BASH_MAC_GNU.value, 'w') as bashMac:
        bashMac.write('some_mac_token=some_mac_value\n')
    with open(env.inputDir + Srcfiles.BASH_LINUX.value, 'w') as bashLinux:
        bashLinux.write('some_linux_token=some_linux_value\n')
    with open(env.inputDir + Srcfiles.BASH_LOCAL.value, 'w') as bashLocal:
    ...
    with open(env.inputDir + Srcfiles.GITCONFIG.value, 'w') as gitConfig:
        gitConfig.write('some_gitconfig_token=some_git_config_value')
    with open(env.inputDir + Srcfiles.GITCONFIG_LOCAL.value, 'w') as gitLocal:
        gitLocal.write('some_gitconfig_local_token=some_local_git_value')
    with open(env.inputDir + Srcfiles.VIMRC.value, 'w') as vimrcLocal:
